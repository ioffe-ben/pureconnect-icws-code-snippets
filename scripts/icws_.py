...
# Code snippet description: the following snippet shows how to generate a list of users to roles using ICWS API.
import json
import requests
...
while i < len(json_users['items']):
    try:
        request_user_roles = requests.get(icws_cloud_authentication.baseURL + icws_cloud_authentication.json_connection_response['sessionId'] + '/configuration/users/' + json_users['items'][i]['configurationId']['id'] + '?select=roles', headers=icws_cloud_authentication.header, cookies=icws_cloud_authentication.cookie, verify=False)
        json_user_roles = json.loads(request_user_roles.text)

        members = ''
        ii = 0
        while ii < len(json_user_roles['roles']['effectiveValue']):
            members += json_user_roles['roles']['effectiveValue'][ii]['id'] + ', '
            ii += 1
